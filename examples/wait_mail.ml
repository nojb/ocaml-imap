    ...
    let l =
      List.map
        (fun s ->
          ...
          | exception Not_found -> (String.trim s, ""))
        l
    in
...
let wait_mail host ?port username password mailbox =
  ...
  Lwt_stream.iter_s
    (fun (_seq, { Imap.Fetch.Response.uid; rfc822; _ }) ->
      let digest = M.find uid map in
      Lwt_io.open_file ~mode:Lwt_io.output (Filename.concat "tmp" digest)
      >>= fun oc ->
      Lwt_io.fprint oc rfc822 >>= fun () ->
      Lwt_io.close oc >>= fun () -> Lwt_io.printlf "Message %s saved." digest)
    strm
