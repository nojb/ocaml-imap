...
let capability =
  ...
  let capability () =
    let h = function
      ...
      | _ -> Lwt.return_unit
    in
    run (g ()) (Imap.run (g ()).c Imap.capability) >>= handle h
  in
...
let login =
  ...
  let login user pass = run (g ()) (Imap.run (g ()).c (Imap.login user pass)) >>= handle_unit in
...
let logout =
  ...
  let logout () = run (g ()) (Imap.run (g ()).c Imap.logout) >>= handle_unit in
...
let noop =
  ...
  let noop () = run (g ()) (Imap.run (g ()).c Imap.noop) >>= handle_unit in
...
let authenticate =
  ...
  let authenticate user pass =
    run (g ()) (Imap.run (g ()).c (Imap.authenticate (Imap.plain user pass))) >>= handle_unit
  in
...
let subscribe =
  ...
  let subscribe m = run (g ()) (Imap.run (g ()).c (Imap.subscribe m)) >>= handle_unit in
...
let unsubscribe =
  ...
  let unsubscribe m = run (g ()) (Imap.run (g ()).c (Imap.unsubscribe m)) >>= handle_unit in
...
let list =
  ...
  let list m =
    let h = function
      ...
      | _ -> Lwt.return_unit
    in
    run (g ()) (Imap.run (g ()).c (Imap.list m)) >>= handle h
  in
...
let select =
  ...
  let select condstore m =
    run (g ()) (Imap.run (g ()).c (Imap.select ~condstore m)) >>= handle_unit
  in
...
let examine =
  ...
  let examine condstore m =
    run (g ()) (Imap.run (g ()).c (Imap.examine ~condstore m)) >>= handle_unit
  in
...
let status =
  ...
  let status m atts =
    let h = function
      ...
      | _ -> Lwt.return_unit
    in
    run (g ()) (Imap.run (g ()).c (Imap.status m atts)) >>= handle h
  in
...
let close =
  ...
  let close () = run (g ()) (Imap.run (g ()).c Imap.close) >>= handle_unit in
  ...
  let search uid key =
    let h = function
      ...
      | _ ->
          Lwt.return_unit
    in
    run (g ()) (Imap.run (g ()).c (Imap.search ~uid key)) >>= handle h
  in
...
let enable =
  ...
  let enable caps =
    let h = function
      ...
      | _ -> Lwt.return_unit
    in
    run (g ()) (Imap.run (g ()).c (Imap.enable caps)) >>= handle h
  in
